	...
	<div>
			...
			<v-tabs-items v-model="userProfileTabs" id="custom-tab-items">
				...
				<v-tab-item value="tabs-info-membresia">
					...
					<v-card class="rounded-xl" color="transparent">
						<v-card-title
							class="text_title_white text_entrena txt_uppercase mb-6"
						>
							MIS PLANES
						</v-card-title>
						<v-row class="px-5">
											...
											<tbody>
                                                ...
                                                mdi-cancel
														...
														<v-btn
															...
															v-if="
																item.status == true &&
																	item.renovacion_automatica == 0 &&
																	checkPlanRenew(item.id_plan) == true
															"
														>
															<v-icon dark small>
																mdi-check
					...
					<v-dialog v-model="dialogDelete" max-width="500px">
						<v-card>
							...
							<v-card-text
								>Estas a punto de cancelar la renovación automatica de tu plan.
								Luego del {{ exp_date_pop | formatDate }} no tendrás más acceso
								a la plataforma. <br /><br />
		...
		<v-dialog v-model="dialogReactivate" max-width="500px">
			<v-card>
				...
				<v-card-actions>
					<v-btn color="error" text @click="dialogReactivate = false"
						><v-icon dark small>
							mdi-close
						</v-icon>
						No</v-btn
					>
					<v-btn color="success" text @click="reactivateSuscripcion()"
						><v-icon dark small>
							mdi-check
						</v-icon>
						Si</v-btn
	...
	methods: {
		...
		async reactivateSuscripcion() {
			try {
				...
				const response = await this.$API.business_partner.reactivateSuscription(
					this.react_id_susc
				);
				this.$store.commit("loader", false);
