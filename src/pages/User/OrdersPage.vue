        ...
        <v-tab-item value="mi-cuenta">
          <div class="my-5">
            <v-card class="my-10 rounded-xl pa-10">
              ...
              <v-card-title class="tit_h1_staff_pink text_entrena txt_uppercase mb-6">
                SEGURIDAD
              </v-card-title>
              <v-row class="px-5">
                <v-col>
                      ...
                      <v-row no-gutters>
                        <v-col cols="12" md="6">
                          <div class="d-flex flex-column">
                            <v-text-field v-model="last_password" :append-icon="
                              showLastPassword ? 'mdi-eye' : 'mdi-eye-off'
                            " :rules="[rules.required]" :type="showLastPassword ? 'text' : 'password'" :error="error"
                              :error-messages="errorMessages" name="password" label="Contraseña Anterior"
                              @click:append="showLastPassword = !showLastPassword"></v-text-field>

                            <v-text-field v-model="new_password" :append-icon="
                            ...
                            <v-text-field v-model="new_password_confirm" :rules="pwdConfirm" :error="error"
                              :error-messages="errorMessages" type="password" name="password"
                              label="Confirmar contraseña (min. 8 caracteres)"></v-text-field>
                          </div>
                        </v-col>
                      </v-row>

                      <v-btn color="blue-grey darken-4" class="mt-2 mb-2" @click="submitChangePassword"
                        :loading="loadingChangePassword" outlined>
          ...
          <div class="my-2">
            <v-card class="my-10 rounded-xl pa-10 orders-main-container" :loading="loading">
              ...
              <v-card-text class="orders-container" v-if="hasOrders">
                <v-card v-for="(order, i) in orders" :key="i" class="order-card elevation-2">
                  <v-card-title style="color: black" class="d-flex flex-row justify-space-between">
                    <span>
                      {{ order.created_at.split("T")[0] }}
                    </span>

                    <v-chip class="ma-2" :color="getColor(order.order_status.id)" label text-color="white">
                  ...
                  <v-card-text class="d-flex flex-column">
                    <v-row>
                      <v-col cols="12" sm="12" md="12">
                        <v-text-field readonly background-color="white lighten-5" outlined label="Metodo de pago"
                          hide-details="auto" :value="order.payment_method.name"></v-text-field>
                      </v-col>
                    </v-row>
                    <v-row>
                      <v-col cols="12" sm="6" md="6">
                        <v-text-field readonly background-color="white lighten-5" outlined label="Subtotal"
                          hide-details="auto" :value="order.subtotal"></v-text-field>
                      </v-col>
                      <v-col cols="12" sm="6" md="6">
                        <v-text-field readonly hide-details="auto" background-color="white lighten-5" outlined
                          label="IGV" :value="order.igv"></v-text-field>
                      </v-col>
                    </v-row>
                    <v-row>
                      <v-col cols="12" sm="12" md="12">
                        <v-text-field readonly background-color="white lighten-5" outlined label="Total"
                          :value="order.total"></v-text-field>
                      </v-col>
                    </v-row>
                    <div class="
                        d-flex
                      ...
                      <v-btn depressed color="primary" @click="openDetails(order)">
                        Detalles
                        <v-icon class="ml-2"> mdi-eye </v-icon>
                      </v-btn>
                      <v-btn depressed link :to="'/confirmar-pago/' + order.hash" color="green"
                        v-if="order.order_status.id == 5 && order.id_payment_method == 2">
                        Pagar
                        <v-icon class="ml-2"> mdi-check-circle </v-icon>
                      </v-btn>

                      <v-btn depressed link :to="'/pago-payme/' + order.hash" color="green"
                      ...
                      <a v-else-if="order.order_status.id == 1 && order.invoice != null"
                        :href="'https://lavikinga.bytesoluciones.net/print/document/'+ order.invoice.external_id +'/a4'"
                        style="text-decoration:none;" target="_blank" rel="invoice">
            ...
            <v-dialog v-model="dialog" max-width="900">
              <v-card>
                <v-card-title class="d-flex flex-row justify-space-between align-center">
                  ...
                  <v-chip class="ma-2" :color="getColor(selectedOrder.order_status)" label text-color="white">
                ...
                <v-card-text>
                  <v-row>
                    <v-col cols="12" sm="12" md="12">
                      <v-text-field readonly background-color="white lighten-5" outlined label="Metodo de pago"
                        hide-details="auto" :value="selectedOrder.payment_method"></v-text-field>
                    </v-col>
                  </v-row>

                  <v-row>
                    <v-col>
                      <v-data-table :headers="headers" :items="selectedOrder.detail" :hide-default-header="false"
                        :hide-default-footer="true" class="elevation-1"></v-data-table>
                    </v-col>
                  </v-row>
                  <v-row>
                    <v-col cols="12" sm="6" md="6">
                      <v-text-field readonly background-color="white lighten-5" outlined label="Descuento"
                        hide-details="auto" :value="selectedOrder.descuento"></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="6" md="6">
                      <v-text-field readonly background-color="white lighten-5" outlined label="Subtotal"
                        hide-details="auto" :value="selectedOrder.subtotal"></v-text-field>
                    </v-col>
                  </v-row>
                  <v-row>
                    <v-col cols="12" sm="6" md="6">
                      <v-text-field readonly background-color="white lighten-5" outlined label="IGV" hide-details="auto"
                        :value="selectedOrder.igv"></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="6" md="6">
                      <v-text-field readonly background-color="white lighten-5" outlined label="TOTAL"
                        hide-details="auto" :value="selectedOrder.total"></v-text-field>
                    </v-col>
                  </v-row>
                </v-card-text>
                <v-card-actions class="d-flex flex-wrap justify-end">
                  ...
                  <v-btn depressed link :to="'/pago-payme/' + selectedOrder.hash" color="green"
                    v-if="selectedOrder.order_status == 5 && selectedOrder.payment_method_id == 1">
                    Pagar
                    <v-icon class="ml-2"> mdi-check-circle </v-icon>
                  </v-btn>
                  <v-btn depressed link :to="'/confirmar-pago/' + selectedOrder.hash" color="green"
                    v-else-if="selectedOrder.order_status == 1 && selectedOrder.payment_method_id == 1">
                    Pagar
                    <v-icon class="ml-2"> mdi-check-circle </v-icon>
                  </v-btn>
                  <a v-else-if="selectedOrder.order_status == 1 && selectedOrder.invoice_external_id != null"
                ...
                <v-col>
                    <v-row class="px-5">
                      <v-col>
                        ...
                        <v-btn @click="openTarjetaAdd()" class="mx-2" color="error" v-if="show_btn_add == true">
                          <v-icon dark>
                            mdi-plus
                          </v-icon> Nueva Tarjeta
                        </v-btn></p>
                        <div class="d-flex">
